...
export const i18nServiceMockFactory = (catalog: ITranslations = ptBR): ObjetoEspiaoJest<I18nService> => {
  const locale = signal<Locale>('pt-BR');
  const catalogSignal = signal<ITranslations>(catalog);

  const mock = criarObjetoEspiaoJest<I18nService>(['bootstrap', 'setLocale', 't', 'pageTitle'], {
    locale: locale.asReadonly(),
    catalog: catalogSignal.asReadonly(),
  });

  mock.t.mockImplementation(<K extends TranslationKey>(key: K, ...args: string[]) => {
